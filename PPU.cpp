...
PPU::PPU(){
    ...
    ppu_ctrl = MMU::getInstance()->read(&addr);

    // Point ppu_mask to the PPUMASK register
    addr = PPU_MASK;
    ppu_mask = MMU::getInstance()->read(&addr);

    // Point ppu_status to the PPUSTATUS register
    addr = PPU_STATUS;
    ppu_status = MMU::getInstance()->read(&addr);
...
void PPU::dataPort(uint8_t *value){
    ...
    bool vram_increment_flag = ((*ppu_mask & 0x04) == 0x04);
    uint16_t increment_amount = vram_increment_flag ? 1 : 32;
